...
public class CalculatePower {
    ...
    public static double getCalculatePower(List<Task> taskList, List<Vmware> vmwareList){
 ...
 */
            ...
            for (int currentTime=minAt;currentTime<=(maxFt-minAt);currentTime++){//当前虚拟机开始从第一个任务到达的时间起开始算，一秒一秒算当前的利用率
                ...
                for (int j=0;j<vmwareList.get(i).getTaskList().size();j++){//一个一个取任务
                    ...
                    if (currentTime>=currentTask.getStartTime()&&currentTime<=currentTask.getFinishTime()){//获取当前正在运行的任务的利用率
                        ...
                        if (currentTask.getNeedUtilization()>20){//程序在运行，并且利用率大于20
                            currentSecondUtlization+=currentTask.getNeedUtilization();
                        }
                        else {
                            currentSecondUtlization+=20;
                        }
                    }else {//如果当前没有运行的任务，就是在空运行，利用率就是20也就是idel
                        currentSecondUtlization+=20;
                    }
                ...
                currentVmPower=caculateCurrentUtPowerWithETC(currentSecondUtlization);
    ...
    public static double caculateCurrentUtPowerWithETC(double utilzation){
        double[] watts=new double[]{0,0,78.5,83,85,88,93,102,109,122,136};
        int unit=0;//利用率的个位数
        int digit=0;//利用率的十位数
        int i;//记录能耗取到哪里了

        unit= (int) (Math.ceil(utilzation%10));//取个位数
        digit=(int)utilzation/10;//取十位数
        ...
        if(unit!=0){//如果个位不为0就计算个位
            count+=watts[i]/10*unit;//算个位的能耗
        }
